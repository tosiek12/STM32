...
#include "SDCard/fatfs/ff.h"			/* Declarations of FatFs API */
#include "SDCard/fatfs/diskio.h"		/* Declarations of disk I/O functions */
	...
	wbuff += wcnt, fp->fptr += wcnt, *bw += wcnt, btw -= wcnt) {
		if ((fp->fptr % SS(fp->fs)) == 0) { /* On the sector boundary? */
			...
			if (fp->dsect != sect) { /* Fill sector cache with file data */
				if (fp->fptr < fp->fsize && disk_read(fp->fs->drv, fp->buf, sect, 1))
					ABORT(fp->fs, FR_DISK_ERR);
			}
#endif
			fp->dsect = sect;
...
FRESULT f_rename(const TCHAR* path_old, /* Pointer to the object to be renamed */
							...
							if (!dw) {
								...
								if (res == FR_OK && dir[1] == '.') {
									dw = (djo.fs->fs_type == FS_FAT32
											&& djn.sclust == djo.fs->dirbase) ? 0 : djn.sclust;
									st_clust(dir, dw);
									djo.fs->wflag = 1;
								}
							}
